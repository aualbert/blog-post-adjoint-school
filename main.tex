...
In a previous post, we saw how transition systems can be modeled as coalgebras \(X \to F(X)\), where \(X\) is a set of states and the functor \(F\) gives the flavor of transitions. For well-behaved coalgebras, determinization induces a trace equivalence. Those systems of interest includes \(A\)-automata with \(M\)-effects and observations in \(O\): \(FM\)-coalgebras for the Moore automata functor \(F = O \times (\mathunderscore)^A\), where the monad \(M\) captures the type of transitions.
...
Rather than writing down unit and multiplication explicitly, we can use a \emph{presentation} of \(\mathcal{C}\): an algebraic theory \((\Sigma, E)\) such that \(\mathcal{C}\) is isomorphic to the term monad \(T_{(\Sigma, E)}\) mapping sets of variables to \(\Sigma\)-terms modulo \(E\). Equivalently, the Eilenberg-Moore category of \(\mathcal{C}\)-algebras is concretely equivalent to the category of models of \((\Sigma, E)\): sets \(A\) equipped with operations interpreting the signature symbols that satisfy the axioms.

Handling \(\mathcal{C}\) via a presentation is very convenient, as it gives equational tools to reason about \(\mathcal{C}\). For instance, to determinize an automaton with \(\mathcal{C}\)-effects and observations in \(O\), the set \(O\) must be a \(\mathcal{C}\)-algebra. A presentation tells us exactly what structure \(O\) needs to carry. In the case of $\C$, it is known that \(\mathcal{C}\) is presented by the theory of convex semilattices:

Its signature has a binary operation \(\oplus\) (nondeterministic choice), and for each \(p \in (0,1)\) a binary operation \(+_p\) (probabilistic choice with probability \(p\)). As for the axioms, those are:
\[
\begin{aligned}
&\text{(A)} && x \oplus (y \oplus z) = (x \oplus y) \oplus z \\
&\text{(C)} && x \oplus y = y \oplus x \\
...
\paragraph{Modeling termination.}
The monad \(\mathcal{C}\) alone does not account for termination, but we can combine it with the termination monad \(+1: X \mapsto X \sqcup \{\star\}\), where \(\star\) signals termination. There are two distributive laws between \(\mathcal{C}\) and \(+1\), giving two different combined monads.
...
\section{1-Bounded metric spaces}

%\subsection{Example (why should we care about program distance)}

Why consider metric spaces and distances between programs? Equivalence is often too coarse. We may want to know how different two programs are, e.g., the probability that they produce different outputs, or the `cost' of one optimization over another. Distances provide this quantitative measure. \todo{example}

We saw that we can use the monad $\C$ to reason about systems that combine nondeterminism and probability. Now, in order to capture the notion of distances we will use monads in $\cat{1Met}$ instead of $\cat{Set}$.
...
But how to define useful $\cat{1Met}$ effect monads? A solution is to lift $\cat{Set}$ monads! Given an endofunctor $F \colon \cat{Set} \to \cat{Set}$, if we start with a metric space $(X,d)$ the idea is to endow $F(X)$ with a metric so we can define $\hat F \colon \cat{1Met} \to \cat{1Met}$ using the same actions on objects and on arrows as in $F$. In order to do that with $\C$, since it combines nondeterminism and probability, we first talk briefly about lifting $\mathcal{P}$ and $\mathcal{D}$. 
...
and then the notion of distance between to subsets $A,B \subseteq X$
...
Now, in order to have symmetry, we need to compare the distances $d(A,B)$ and $d(B,A)$, resulting exactly in the definition we wanted for the first image
...
In probability theory, such a function is as a joint probability distribution on $X \times X$ with marginals $P$ and $Q$ and is therefore called a coupling of $P$ and $Q$. Here, we will call it a transportation plan for supply $P$ and demand $Q$ and write $T (P, Q)$ for the set of all such transportation plans.
...
For generics metric space $(X,d)$ and distributions $\varphi_1, \varphi_2 \in \mathcal{D}(X)$, we define
\[K(d) = inf_{t \in Coup(\varphi_1,\varphi_2)} (\sum_{(x_1,x_2) \in X \times X} t(x_1,x_2) \cdot d(x_1,x_2) )\]
where $Coup(\varphi_1, \varphi_2)$ is defined as the collection of couplings of $\varphi_1$ and $\varphi_2$, i.e., $Coup(\varphi_1, \varphi_2) = \{t \in \mathcal{D}(X \times X) : \mathcal{D}(\pi_1)(t) = \varphi_1 \text{ and } \mathcal{D}(\pi_2)(t) = \varphi_2\}$ where $\pi_1 : X_1 \times X_2 \to X_1$ and $\pi_2 : X_1 \times X_2 \to X_2$ are the projection functions.
$K(d)$ is known as the \textbf{Kantorovich lifting of $d$}.

So, starting with a 1-bounded metric space $(X,d)$ we have that $(\mathcal{D}(X),K(d))$ is also a 1-bounded metric space.

Using these two liftings we obtain that $(C(X), HK(d))$ is a 1–bounded metric space. This leads to the definition of the $\cat{1Met}$ monad $\hat \C$, which has the same action on arrows and same unit and multiplication as the monad $\C$.


\paragraph{Presentation.}

Now, the difference in considering program distances instead of program equivalence is expressed also algebraically. Instead of using the equality ``$=$'' in the axioms, we define an equality relation $a =_{\epsilon} b$ which we think of as saying that ``$a$ is equal to $b$ up to an error of $\epsilon$".  

The concept of equation of universal algebra is replaced by that of quantitative inference 
$$\{x_i =_{\epsilon_i} y_i\}_{i\in I} \vdash s =_{\epsilon} t$$ where $\epsilon$, $\epsilon_i \in [0,1]$ and $s, t$ are terms. We have that the quantitative inference is satisfied if, for all interpretations $\iota$ of the variables $x$, the following holds:

\[ \text{if } d(\iota(x_i), \iota(y_i)) \leq \epsilon_i \text{ for all } i \in I, \text{ then } (\iota(s), \iota(t)) \leq \epsilon.\]

\begin{definition}
    The quantitative equational theory \(\QTh_{CS}\) of convex semilattices has signature $\Sigma_CS = (\{\oplus\}\cup\{+_p\}_{p\in(0,1)})$ and is defined as the set of quantitative inferences derivable by the following axioms, stated for arbitrary $p, q \in (0, 1)$ and $\epsilon_1, \epsilon_2 \in [0, 1]$:
    We substitute every axiom $A,C,I,A_p,C_p,I_p$ and $D$ in \(\Th_{CS}\) by changing the usual equality ``$=$'' for the inference $ \emptyset \vdash $ with $=_0$ and add the following inferences
    \[\begin{aligned}
...
\paragraph{Liftings with termination.}

we see that we can also automatically define a $\hat C(+ \hat 1)$ monad.

\todo{$\hat C(+ \hat 1)$ can be defined just as before, use the general monad distribution law.}

So what about the other monads, can they be lifted ? Or alternatively, is there a monad for the other quantitative algebras?

\todo{adapt this:
negative results by showing that any such monad:
1) cannot have a multiplication defined as the one of the
Set monad C + 1, and
2) cannot be presented by the quantitative theory of convex
semilattices with bottom and black–hole, since this theory
is trivial}

\todo{Mention very quickly $\hat \C \downarrow$ without going into the details since we don't want to talk about the monad laws in details.}
