...
% Consider, for example, the monad $\mathcal{C}$ of non-empty finitely generated convex sets of probability distributions. Intuitively, $\mathcal{C}$ models non-deterministic choice followed by probabilistic choice. (Note that $\mathcal{C}$ is not simply the composition of the classical monads for non-determinism and probability choice, since there is no distributive law to compose them.)
...
In a previous post~\todo{link to the blog by Alyssa and Max}, we learned how transition systems could be modeled as coalgebras \(S \to F(S)\), with \(S\) a set of states, and \(F\) a functor. In addition, we saw how to obtain a good notion of equivalence for those systems: determinization followed by bisimulation! Of all the systems one could think of, automata strike a good balance: they are very expressive, while still being described by an unifying theory.  
An \(A\)-automata with \(M\)-effects and observations in \(O\) is a coalgebras $S \to O \times (MS)^A$. The set $O$ describes all the observations one can make about a state in $S$, while functor $M$ gives some flavor to the transitions... 
...
Rather than writing down unit and multiplication explicitly, we will use a \emph{presentation} of \(\mathcal{C}\): an algebraic theory \((\Sigma, E)\) such that \(\mathcal{C}\) is isomorphic to the term monad \(T_{(\Sigma, E)}\) mapping sets of variables to \(\Sigma\)-terms modulo \(E\). Equivalently, the Eilenberg-Moore category of \(\mathcal{C}\)-algebras is concretely equivalent to the category of models of \((\Sigma, E)\) (e.g. models of the theory of rings are simply rings!).
...
For instance, to determinize an automaton with \(M\)-effects and \(O\)-observations, we need an algebra \(MO \to O\). A presentation tells us exactly what structure \(O\) must to carry! 

For $\C$, it was formally proved in~\todo{cite paper} that this monad is presented by the theory of convex semilattices, whose signature
...
With a presentation of $\mathcal C$, we get a language to describe the new states of the determinization of a $\mathcal C$-automata: each determinized state is a finite expression like  $\mathtt{Pacman}$ or $\mathtt{win} +_{3/4} \mathtt{lose}$, and the presentation tells us how to compute the transitions:
For ``simples'' states like $\mathtt{Pacman}$, we just need to provide the expression corresponding to the right combination of nondeterminism and probability:
 \begin{align*}
     &\mathtt{Pacman}\\
...
For ``complex'' states like the one above, we just replace every occurrences of simple states by the complex states we get after transitioning from these simple states: 
...
Finally, we can use the equations of the presentation to simply this expression a bit:
...
The way we modeled termination with a terminal state $\star$ was a bit ad hoc, though. A more systematic way of modeling, which also behaves better with complex sets of observations, is to compose a monad $M$ modeling certain effects with the termination monad \(+1: X \mapsto X \sqcup \{\star\}\), where \(\star\) signals termination. Indeed, there is always a monad distribution law \(\iota \colon M+1 \Rightarrow M(+1)\). For $\mathcal C$, this gives the monad \(\mathcal{C}(+1)\) of non-empty finitely generated convex sets of \emph{subdistributions}. Since \(\mathcal{C}(+1)\) is again a monad, we can then replace $\mathcal C$ by \(\mathcal{C}(+1)\) in our algebraic formulation, and everything works! 
Interestingly, this monad is presented by the theory of \emph{pointed} convex semilattices, i.e., we add an extra symbol $\star$ in the signature. 

However, there are different ways to model termination with the monad $\mathcal C$, for example termination may be a nondeterministic alternative: a state either transition to an non-terminating effectful state, or terminates outright. This is another point where presentation shines: it is easy to add additional axioms to the theory of pointed convex semilattices to refine the meaning of termination. However, we are not guaranteed that these modified theory still presents a monad... Nevertheless, it does when adding either the bottom axiom $(\bot)$, or both $(\bot)$ and the black hole axiom $(BH)$!
\begin{align*}
    &\bot = \{x \oplus \star = x\} &BH = \{x +_p \star = \star \mid p \in (0, 1)\}
\end{align*}
...
% \]
% Here, \(\mathbf{nil}\) represents the terminated process, \(a.P\) performs action \(a\) and continues as \(P\), and  \(\overline{\oplus}\) and \(\overline{+}_p\) are non-deterministic and probabilistic branchings, respectively.
...
\section{From equivalences to distances between automatons}

%\subsection{Example (why should we care about program distance)}
Now here comes the best part: while equivalences of programs are nice, distance between programs are even nicer, and sometimes necessary to reason properly about programs with non-determinism! Our intuition tells us that programs that return the same output with high-probability should be closer than programs whose output are different almost all the times. A distance between programs quantify this intuition.
...
Consider for example the following transition system, without labels, and with state space $X = \{u,x,y,z\}$, where $z$ is a final state and $u$ is a fail state, which once reached can never be left and the system loops indefinitely.
...
If we compare the behavior of $x$ and $y$, we see that they are not behaviorally equivalent. This is because in state $x$ there is always a bias towards the final state $z$ which is controlled by the value of $\epsilon$. But, they are similar in the sense that their behavioral distance is $\epsilon$. If we took $\epsilon = 0$, then they would really be behaviorally equivalent.
...
We now need some $\cat{1Met}$ monads to replace the $\cat{Set}$-monads we used for effects. A solution is to lift $\cat{Set}$ monads! Given an endofunctor $F \colon \cat{Set} \to \cat{Set}$, if we start with a metric space $(X,d)$, the idea is to endow $F(X)$ with a metric so we can define $\hat F \colon \cat{1Met} \to \cat{1Met}$ using the same actions on objects and on arrows as in $F$. Before applying this idea to $\C$, lets start by lifting the powerset and probability distribution monads $\mathcal{P}$ and $\mathcal{D}$, modeling respectively probability and non-determinism.
Recall that $\mathcal P$ maps sets $X$ to their powerset $\mathcal P X$, while $\mathcal D$ maps $X$ to the finitely supported probability distributions on $X$.

\paragraph{Lifting of $\C$.}

To lift the functor $\mathcal{P}$ to $\cat{1Met}$ we need a notion of distances between subsets. Given $(X,d)$ a metric space, we talk about the distance between $A, B \subseteq X$ using the metric $d$. The idea is exactly to measure how far $A$ is from $B$ in $X$:
...
\end{figure}
where it's obvious that $A = B$ if and only if this distance is equal to zero.
...
Now, in order to insure symmetry, we need to compare the distances $d(A,B)$ and $d(B,A)$, resulting exactly in the definition we wanted for the first image
\[ H(d)(A,B) = max\{d(A,B),d(B,A)\} \]
$H(d)$ is known as the \textbf{Hausdorff lifting of $d$}.
So, starting with a 1-bounded metric space $(X,d)$, we have that $(\mathcal{P}(X),H(d))$ is also a 1-bounded metric space.

Now, for the functor $\mathcal{D}$ we need a more elaborated example (from \cite{coalg-metrics}) in order to understand the intuition behind the lifting.  

Suppose you are the owner of three coffee roasting plants, $A,B$ and $C$, each with an adjacent café where one can taste the coffee. We let $X = \{A,B,C\}$ and consider the distributions $P \colon X \to [0,1]$ and $Q \colon X \to [0,1]$ below to model supply and demand, respectively, of the product in proportion of the total supply or demand:
...
We have illustrated this situation graphically below. The numbers on the edges indicate the distance between the places $A,B$ and $C$ whereas the numbers on the nodes indicate supply $P$ (upper value) and
...
In probability theory, such a function is a joint probability distribution on $X \times X$ with marginals $P$ and $Q$ and is therefore called a coupling of $P$ and $Q$. Here, we will call it a transportation plan for supply $P$ and demand $Q$ and write $T (P, Q)$ for the set of all such transportation plans.

Since the cost of transporting units from a place to another is directly proportional to the distance between the places, to minimize the costs we want to minimize $\sum_{x,y \in X} t(x,y) \cdot d(x,y)$ among all possible transportation plans. Having that in mind we define
\[ K(d)(P,Q) = min\{\sum_{x,y \in X} t(x,y) \cdot d(x,y) : t \in T (P, Q)\} \]

For a generic metric space $(X,d)$ and distributions $\varphi_1, \varphi_2 \in \mathcal{D}(X)$, we define
\[K(d) = inf_{t \in Coup(\varphi_1,\varphi_2)} (\sum_{(x_1,x_2) \in X \times X} t(x_1,x_2) \cdot d(x_1,x_2) )\]
where $Coup(\varphi_1, \varphi_2)$ is defined as the collection of couplings of $\varphi_1$ and $\varphi_2$, i.e., 
$$Coup(\varphi_1, \varphi_2) = \{t \in \mathcal{D}(X \times X) : \mathcal{D}(\pi_1)(t) = \varphi_1 \text{ and } \mathcal{D}(\pi_2)(t) = \varphi_2\}$$ where $\pi_1 : X_1 \times X_2 \to X_1$ and $\pi_2 : X_1 \times X_2 \to X_2$ are the projection functions.
$K(d)$ is known as the \textbf{Kantorovich lifting of $d$}.
Starting with a 1-bounded metric space $(X,d)$ we thus have that $(\mathcal{D}(X),K(d))$ is also a 1-bounded metric space.

Using these two liftings, we can show that $(C(X), HK(d))$ is a 1–bounded metric space. This gives us our desired $\cat{1Met}$-monad $\hat \C$, which has the same action on arrows and same unit and multiplication as the monad $\C$.

\paragraph{What about presentations?}

Now, the difference in considering program distances instead of program equivalence is expressed also algebraically. Instead of describing the axioms using equality ``$=$'', we use relations $a =_{\epsilon} b$ which we think of as saying that ``$a$ is equal to $b$ up to an error of $\epsilon$".  

Without equalities, we replace sequences of equalities by quantitative inferences:
$$\{x_i =_{\epsilon_i} y_i\}_{i\in I} \vdash s =_{\epsilon} t$$ where $\epsilon$, $\epsilon_i \in [0,1]$ and $s, t$ are terms. A quantitative inference is satisfied if, for all interpretations $\iota$ of the variables $x$, we have:

\[ \text{if } d(\iota(x_i), \iota(y_i)) \leq \epsilon_i \text{ for all } i \in I, \text{ then } (\iota(s), \iota(t)) \leq \epsilon.\]

Formally, we replace algebraic theories by quantitative equational theories. For instance, the quantitative equational theory of convex semilattices has signature $\Sigma_CS = (\{\oplus\}\cup\{+_p\}_{p\in(0,1)})$ and is defined as the set of quantitative inferences derivable by the following axioms, stated for arbitrary $p, q \in (0, 1)$ and $\epsilon_1, \epsilon_2 \in [0, 1]$:
...
\end{aligned}\]
    ...
    \[\begin{aligned}
&\text{(H)} && \{x_1 =_{\epsilon_1} y_1, x_2 =_{\epsilon_2} y_2\} \vdash x_1 \oplus x_2 =_{max(\epsilon_1,\epsilon_2)} y_1 \oplus y_2 \\
&\text{(K)} && \{x_1 =_{\epsilon_1} y_1, x_2 =_{\epsilon_2} y_2\} \vdash x_1 +_p x_2 =_{p \cdot \epsilon_1 + (1-p) \cdot \epsilon_2} y_1 +_p y_2 
\end{aligned}\]


\paragraph{Lifting termination.}

As in the $\cat{Set}$ case, the termination monad $+\hat 1$ on $\cat{1Met}$ also admits a monad composition law with any  $\cat{1Met}$-monad, and we thus get a monad $\hat C(+ \hat 1)$ for free, which is also nicely presented by the quantitative equational theory of \emph{pointwise} convex semilattices, mirroring the $\cal{Set}$-case! For the $\mathcal C^\downarrow$ monad, much more work is needed to show that there is a $\cat{1Met}$-monad $\hat{\mathcal C^\downarrow}$ presented by quantitative equational theory of pointed convex semilattices with the additional quantitative inference $(\bot_Q) = \{\vdash x \oplus \star =_0 x\}$. As for the $\mathcal C+1$ monad, however, if we also add the quantitative inference $(BH_Q) = \{\emptyset \vdash x+_p \star =_0 \star \mid  p \in (0, 1)\}$, then the result theory collapses: the quantitative $\{x =_\epsilon y\}_{\epsilon>0} \vdash x =_0 y$ is derivable.
